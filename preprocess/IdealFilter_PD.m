...
spm('defaults','fmri');
wkdir = '/g5/kirby/xmiao5/XZhou86/Model free method';  % enter data path
...
for n=1:length(all)%[(1:15),(17:length(all))]
    ...
    cd(datadir)
    x= spm_select('FPlist',pwd,'^filter');
    if isempty(x)
    ...
    F = spm_select('FPlist',pwd,'.*8_1.nii'); %input denoised.nii
    F2 = spm_select('FPlist',pwd,'denoised.nii'); 
%     if isempty(F)
%         copyfile('filtered_func_data.nii.gz')
%     end
    
    functmp = load_untouch_nii(F);
    func = single(functmp.img);
    ss = size(func);%width,length,time
    % Convert into 2D: time series * voxel(flatten)
    AllVolume = reshape(func,[],ss(4))';
    % delete 24 frames which may cause the memory issue
    AllVolume = AllVolume(25:300,:);
    ss(4) = 276;
...
%     save_untouch_nii(functmp,save_name);
    save_fakeuntouch_nii(functmp,save_name);
